...
| `SCENES_FILE`    | `scenes.json`      | Path to the scenes definition file   |
...
Scenes are predefined event payloads stored in `scenes.json`.  
Each scene has a `name`, `description`, detection list, and a `photo` field (currently `null` – will hold image references later).
